...
%        \vspace*{1em}
%        \item[$\Rightarrow$] Scan over thousands of hyperparameter combinations and select the best one
...
%\end{enumerate}
%
%\only<1-4>{{ Result: $\mathrm{PDF}=\text{NN}(x')-\text{NN}(1)$}}
%%\only<5>{\alert{ Result: $\mathrm{PDF}={A} \text{NN}(x')$}}
%\only<5>{\alert{ Result: $\mathrm{PDF}= \text{NN}(x')-\text{NN}(1)$}}
%
%\column{0.40\linewidth}
%\only<5>{\\
%\textbf{Pro:} Simpler architecture\\
...
%    \textbf{Challenges:}
    \begin{itemize}
%        \item Generating more precise predictions
