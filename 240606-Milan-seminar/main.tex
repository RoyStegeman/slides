...
\begin{frame}{DIS coefficient functions}
  \begin{itemize}
    ...
    \item Massive coefficient functions can be constructed by smoothly joining the known limits from high energy and threshold resummations and the massless limit ($Q^2 \rightarrow m_h^2$, $x\rightarrow 0$, and $Q^2\gg m_h^2$) {\color{gray}\footnotesize [Barontini, Bonvini, Laurenti, in preparation]}
  \end{itemize}
  ...
  \begin{columns}
    ...
    \begin{column}{0.49\textwidth}
      ...
      \begin{figure}
        ...
        \caption*{Validate the procedure at NNLO}
      \end{figure}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Theory uncertainties in PDFs}
  ...
  \begin{itemize}
    ...
    $$P(T|D) \propto \exp\left[-\frac{1}{2}\left(T-D\right)C^{-1}\left(T-D\right)\right] \propto \exp\left[-\frac{1}{2}\chi^2\right]$$
...
\begin{frame}{Phenomenological impact}
  ...
  \begin{itemize}
    ...
    \item Photon induced effects are up to 5\% in the high $Q^2$ region (blue vs orange)
...
\begin{frame}{$\alpha_s$ from correlated theory uncertainties}
  ...
  ${\color{green} P(T|D)} \propto \int d\lambda \exp\left[Z^{-1}\left(\lambda-\bar{\lambda}\right)^2\right] \exp\left[-\frac{1}{2}\left(T-D\right)^T\left(\mathrm{Cov}_\mathrm{exp}+\beta\beta^T\right)^{-1}\left(T-D\right)\right]$ \\
  for some functions $Z$ and $\bar{\lambda}$,

  % \vspace*{1em}
  % Taking $\beta\beta^T=\mathrm{Cov_\mathrm{th}}$, we thus recover \\
  ...
  $$P(\lambda|T,D) = \frac{{\color{red}P(T|D,\lambda) }{\color{blue} P(\lambda)}}{{\color{green} P(T|D)}}=\exp\left[-\frac{1}{2}Z^{-1}(\lambda - \bar{\lambda})\right]$$
